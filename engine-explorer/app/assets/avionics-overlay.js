...
function sync(){syncLoading();root.querySelectorAll('input[type="range"][aria-label="Crank angle"]:not([data-av-rotary])').forEach(function(input){input.dataset.avRotary="";input.dataset.avUnit="°";window.SuarezTactile?.refresh()});var comparison=root.querySelector('.ignition-lab .lab-differences');if(comparison&&!comparison.hasAttribute('tabindex')){comparison.tabIndex=0;comparison.setAttribute('role','region');comparison.setAttribute('aria-labelledby','lab-comparison');var hint=document.createElement('p');hint.className='lab-scroll-hint';hint.textContent='Scroll sideways to compare both ignition systems';comparison.querySelector('h3')?.after(hint)}var exploring=!!root.querySelector('.app.explorer .edge-controls');dock.hidden=!exploring;document.body.classList.toggle('cc-exploring',exploring);if(!exploring){menu.open=false;return}
