...
var dock=document.createElement('nav');dock.className='explorer-dock';dock.setAttribute('aria-label','Explore controls');dock.hidden=true;
...
menu.innerHTML='<summary><svg class="av-icon" aria-hidden="true"><use href="/assets/avionics-icons.svg#Menu"></use></svg><span>Menu</span></summary><div class="explorer-menu-panel"><p class="explorer-menu-heading">Explore controls</p><div class="explorer-menu-actions" role="group" aria-label="Explore controls"></div><div class="explorer-appearance"><p>Appearance</p><div role="group" aria-label="Appearance"><button type="button" data-preference="dark">Dark</button><button type="button" data-preference="light">Day</button><button type="button" data-preference="system">System</button></div><label><input type="checkbox"> Solid controls</label></div></div>';
var actions=menu.querySelector('.explorer-menu-actions');
var mirrors=new Map();
function create(label,where){var b=document.createElement('button');b.type='button';b.className='rail-button';b.setAttribute('aria-label',label);b.dataset.action=label;where.appendChild(b);mirrors.set(label,b);b.addEventListener('click',function(){var original=find(label);if(!original||original.disabled)return;original.click();menu.open=false;requestAnimationFrame(sync)})}
...
function sync(){syncLoading();var exploring=!!root.querySelector('.app.explorer .edge-controls');dock.hidden=!exploring;document.body.classList.toggle('cc-exploring',exploring);if(!exploring){menu.open=false;return}
 ...
 mirrors.forEach(function(b,label){var original=find(label);b.hidden=!original;b.disabled=!original||original.disabled;if(!original)return;b.innerHTML=original.innerHTML;b.setAttribute('aria-label',original.getAttribute('aria-label')||label);var expanded=original.getAttribute('aria-expanded');if(expanded!==null)b.setAttribute('aria-expanded',expanded);else b.removeAttribute('aria-expanded')});
