...
All chrome text uses an embedded bold TTF (fonts/Inter-700.ttf via the
CHROME_FONT helpers — core-14 Helvetica is never embedded and breaks
fit_size() under viewer substitution). The final save uses
garbage=4, deflate=True, clean=True so the stamped file stays smaller
than the clean base (idea-48).
...
CHROME_FONT_FILE = os.path.join(HERE, "fonts", "Inter-700.ttf")
...
def main():
    ...
    # garbage=3, which bloated the file by ~48%).
    doc.subset_fonts()
