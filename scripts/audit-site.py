...
# Explicit built app documents that are shipped as runtime entry points, not
...
def runtime_entry_reason(rel):
    return BUILT_RUNTIME_ENTRIES.get(rel)
...
def main():
    ...
    for f in sorted(discovered_set-manifest_files):
        ...
        rel = str(f.relative_to(ROOT))
        if not re.search(r'url\s*=\s*[^;]+',q.refresh,re.I) and not runtime_entry_reason(rel): fail('unmanifested_route','/'+rel,'HTML route is outside config/site-pages.json')
    ...
    report={'generated_by':'scripts/audit-site.py','origin':manifest['origin'],'summary':{'manifest_routes':len(pages),'discovered_html':len(discovered),'audit_rows':len(rows),'alias_count':len(aliases),'built_runtime_entry_count':sum(row['kind']=='built_runtime_entry' for row in rows),'failure_count':len(failures),'broken_reference_count':len(source_failures),'jsonld_failure_count':len(jsonld_failures),'origin_unknown_failure_count':len(failures)},'aliases':aliases,'failures':failures,'rows':sorted(rows,key=lambda x:x['route'])}
    (ROOT/'docs').mkdir(exist_ok=True); (ROOT/'docs/site-route-audit.json').write_text(json.dumps(report,indent=2,ensure_ascii=False)+'\n')
    runtime_count=sum(row['kind']=='built_runtime_entry' for row in rows)
    md=['# Site route audit','',f"Generated by `scripts/audit-site.py` (read-only source audit). Finding origin is **unknown** without a trusted baseline; this report makes no pre-existing or introduced claim.",'',f"Routes in manifest: **{len(pages)}**",f"HTML files discovered: **{len(discovered)}**",f"Redirect aliases: **{len(aliases)}**",f"Built runtime entries outside the route manifest: **{runtime_count}**",f"Failure count: **{len(failures)}**",f"Broken references: **{len(source_failures)}**",f"Invalid JSON-LD blocks: **{len(jsonld_failures)}**",'']
    if failures:
