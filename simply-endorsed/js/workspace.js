(() => {
  ...
  function tasks() {
    ...
    return `<div class="se-heading"><h1 id="se-title" tabindex="-1">Checklists</h1><p>Choose a task, then check off each requirement.</p></div><div class="se-task-grid">${M.groups.map((g, i) => `<a href="${E(url({ view: "tasks", group: g.id === "new" ? "" : g.id, task: g.id === "new" ? "new-student" : "", detail: "" }))}" data-route class="se-task-card"><h2>${E(g.title)} <span aria-hidden="true">↗</span></h2><p>${E(g.description)}</p></a>`).join("")}</div><div class="se-bottom-links">${a("Browse all 96 endorsements", { view: "library", category: "all", path: "", detail: "" })}${a("Open instructor guidance", { view: "guidance", mode: "journey", topic: "", detail: "" })}</div>`;
  }
